...
class NeuSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [
        BoxShadow(
          color: const Color(0xFF17202D).withValues(alpha: .82),
          offset: const Offset(2, 2),
          blurRadius: 16,
        ),
        BoxShadow(
          color: const Color(0xFF3D4A5E).withValues(alpha: .72),
          offset: const Offset(2, 2),
          blurRadius: 14,
        ),
      ],
...
class _NeuButtonState extends State<NeuButton> {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: widget.onPressed == null ? kNeuSurfaceDeep : kNeuAccent,
        borderRadius: BorderRadius.circular(17),
        border: Border.all(
          color:
              widget.onPressed == null
                  ? Colors.white.withValues(alpha: .04)
                  : kNeuAccentSoft.withValues(alpha: .6),
        ),
        boxShadow:
            _pressed
                ? [
                  BoxShadow(
                    color: const Color(0xFF17202D).withValues(alpha: .65),
                    offset: const Offset(2, 2),
                    blurRadius: 7,
                  ),
                ]
                : [
                  BoxShadow(
                    color: const Color(0xFF17202D).withValues(alpha: .8),
                    offset: const Offset(2, 2),
                    blurRadius: 12,
                  ),
                  BoxShadow(
...
class NeuIconButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    decoration: BoxDecoration(
      color: kNeuSurfaceLight,
      shape: BoxShape.circle,
      border: Border.all(color: Colors.white.withValues(alpha: .045)),
      boxShadow: [
        BoxShadow(
          color: const Color(0xFF17202D).withValues(alpha: .78),
          offset: const Offset(2, 2),
          blurRadius: 10,
        ),
        BoxShadow(
          color: const Color(0xFF3D4A5E).withValues(alpha: .62),
          offset: const Offset(-2, -2),
          blurRadius: 8,
        ),
      ],
    ...
    child: IconButton(
      onPressed: onPressed,
      icon: Icon(icon),
      color: active ? kNeuAccentSoft : Colors.white70,
    ),
  );
}
