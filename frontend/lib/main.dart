...
class _SongDownloaderAppState extends State<SongDownloaderApp> {
  ...
  late final MusicPlayerService _musicPlayer = MusicPlayerService(
    handler: widget.musicHandler,
  );
  ...
  Future<void> _initShareHandling() async {
    ...
    _setSharedUrl(await _shareIntent.getInitialSharedText());
  ...
  void _setSharedUrl(String? text) {
    ...
    if (url != null && mounted) setState(() => _pendingSharedUrl = url);
  }

  void _consumeSharedUrl() {
    if (mounted) setState(() => _pendingSharedUrl = null);
  }
  ...
  Future<void> _initApp() async {
    ...
    for (var attempt = 1; attempt <= 4; attempt++) {
      try {
        ...
        final manager = DownloadManagerService(apiService: api)..start();
        if (!mounted) {
          manager.dispose();
          return;
        }
  ...
  void _openLibrary() {
    if (mounted) _mainShellKey.currentState?.openLibrary();
  }
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    theme: ThemeData(
      ...
      scaffoldBackgroundColor: kNeuBackground,
      colorScheme: ColorScheme.fromSeed(
        seedColor: kNeuAccent,
        brightness: Brightness.dark,
      ).copyWith(
        primary: kNeuAccent,
        secondary: kNeuAccent,
        surface: kNeuSurface,
      ...
      inputDecorationTheme: InputDecorationTheme(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(17),
          borderSide: const BorderSide(color: kNeuAccent, width: 1.2),
        ),
      ),
      filledButtonTheme: FilledButtonThemeData(
        style: FilledButton.styleFrom(
          backgroundColor: kNeuAccent,
      ...
      navigationBarTheme: NavigationBarThemeData(
        backgroundColor: kNeuSurface,
        elevation: 0,
        indicatorColor: kNeuAccent.withValues(alpha: .18),
    ...
    home:
        _initialized
            ? MainShell(
              ...
              onSharedUrlConsumed: _consumeSharedUrl,
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      hasError
                          ? error!
                          : 'Preparing your private downloader on this device.',
                      textAlign: TextAlign.center,
                      style: const TextStyle(color: Colors.white60),
                    ),
                    if (hasError) ...[
                      const SizedBox(height: 20),
                      NeuButton(onPressed: onRetry, child: const Text('Retry')),
                    ] else ...[
  ...
  Widget build(BuildContext context) {
    final pages = [
      HomeScreen(
        ...
        onSharedUrlConsumed: widget.onSharedUrlConsumed,
