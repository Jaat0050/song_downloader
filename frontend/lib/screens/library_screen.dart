...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _playSong(HistoryItem item) async {
    ...
    if (!await file.exists()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('File not found on device storage.')));
      }
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.refresh_rounded), onPressed: _loadLibrary)],
      ),
      body: SafeArea(
        child: Column(
          children: [
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: isPlayingThis ? const Color(0xFF8B5CF6).withValues(alpha: 0.5) : Colors.white.withValues(alpha: 0.05)),
                            ),
                            child: ListTile(
                              ...
                              title: Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                              subtitle: Text(
                              ...
                              trailing: Row(
                                ...
                                children: [
                                  IconButton(
                                    icon: Icon(isPlayingThis ? Icons.pause_circle_filled_rounded : Icons.play_circle_fill_rounded, color: const Color(0xFF8B5CF6), size: 32),
  ...
  Widget _buildPlayerBar() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF1F1B2C), border: Border(top: BorderSide(color: Colors.white.withValues(alpha: 0.1)))),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Row(
            children: [
              ClipRRect(
                ...
                child: SizedBox(
                  width: 42,
                  height: 42,
                  child: item.thumbnail.isNotEmpty ? Image.network(item.thumbnail, fit: BoxFit.cover) : const ColoredBox(color: Color(0xFF24242B), child: Icon(Icons.music_note, size: 20)),
                ),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(item.artist, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white60, fontSize: 12)),
                  ],
                ),
              ),
              IconButton(icon: Icon(_isPlaying ? Icons.pause_rounded : Icons.play_arrow_rounded, size: 30), onPressed: () => _playSong(item)),
              IconButton(
