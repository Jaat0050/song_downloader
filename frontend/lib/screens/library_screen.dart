...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  void initState() {
    ...
    _historySubscription = DownloadStorageService.historyChanges.listen(
      (_) => mounted ? _load() : null,
    );
  }
  ...
  Future<void> _favorite(HistoryItem item) async {
    await _library.toggleFavorite(item.id);
    await _load();
  }

  Future<void> _delete(HistoryItem item) async {
    if (widget.musicPlayer.current?.id == item.id)
      ...
      child: NeuSurface(
        child: Row(
          children: [
            ...
            NeuIconButton(
              icon: active && player.playing ? Icons.pause : Icons.play_arrow,
              active: active,
            ...
            children: [
              Row(
                children: [
                  ...
                  NeuIconButton(icon: Icons.skip_next, onPressed: player.next),
                ],
              ),
              Slider(
                value: value,
