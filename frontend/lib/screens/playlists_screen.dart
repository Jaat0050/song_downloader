...
class _PlaylistsScreenState extends State<PlaylistsScreen> {
  ...
  Future<void> _load() async {
    final x = await _storage.loadPlaylists();
    if (mounted) setState(() => _playlists = x);
  }
  ...
  Future<void> _create() async {
    ...
    if (_playlists.containsKey(name)) { _snack('Playlist already exists.'); return; }
    _playlists[name] = [];
  ...
  Future<void> _edit(String name) async {
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setLocal) {
            return AlertDialog(
              title: Text('Songs in $name'),
              content: SizedBox(
                width: double.maxFinite,
                height: 400,
                child: ListView(
                  children: widget.library.map((song) => CheckboxListTile(
    ...
    if (saved == true) {
      _playlists[name] = selected.toList();
      await _storage.savePlaylists(_playlists);
      if (mounted) setState(() {});
    }
  }
  ...
  Future<void> _play(String name) async {
    ...
    for (final id in ids) {
      for (final song in widget.library) {
        if (song.id == id) { songs.add(song); break; }
      }
    }
    if (songs.isEmpty) { _snack('This playlist has no available songs.'); return; }
            ...
            children: _playlists.entries.map((entry) => Card(
              child: ListTile(
                ...
                trailing: PopupMenuButton<String>(
                  ...
                  itemBuilder: (_) => const [
                    PopupMenuItem(value: 'play', child: Text('Play playlist')),
                    PopupMenuItem(value: 'edit', child: Text('Add / edit songs')),
