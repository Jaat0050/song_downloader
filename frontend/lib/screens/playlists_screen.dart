...
class _PlaylistsScreenState extends State<PlaylistsScreen> {
  ...
  Future<String?> _showName() => showDialog<String>(
    context: context,
    builder: (_) => const _CreatePlaylistDialog(),
  );
  Future<void> _delete(String n) async {
    final u = Map<String, List<String>>.from(_playlists)..remove(n);
    await _storage.savePlaylists(u);
    if (mounted) setState(() => _playlists = u);
  }
