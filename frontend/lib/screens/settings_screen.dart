...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _card({required Widget child}) => Container(
    ...
    decoration: BoxDecoration(color: const Color(0xFF15151B), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withValues(alpha: 0.08))),
    child: child,
  );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(onPressed: _loading ? null : () => _refresh(), icon: const Icon(Icons.refresh_rounded), tooltip: 'Refresh status')],
      ),
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _statusRow('Audio output', health?.audioFormat.toUpperCase() ?? '—', icon: Icons.audiotrack_rounded),
            ])),
            const SizedBox(height: 14),
            _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _statusRow('Completed', '${health?.jobs['completed'] ?? 0}', icon: Icons.check_circle_outline_rounded),
            ...
            const SizedBox(height: 14),
            _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('App Status', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              _statusRow('App version', '1.0.0 (Build 1)', icon: Icons.apps_rounded),
              _statusRow('Platform', Platform.operatingSystem, icon: Icons.phone_android_rounded),
