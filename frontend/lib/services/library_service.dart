...
class LibraryService {
  ...
  Future<void> toggleFavorite(String id) async {
    final items = await _storage.loadHistory();
    final updated = items.map((item) => item.id == id ? item.copyWith(favorite: !item.favorite) : item).toList();
  ...
  Future<void> rename(HistoryItem item, String newName) async {
    ...
    if (!await oldFile.exists()) throw StateError('Audio file no longer exists.');
    final dir = oldFile.parent;
    final target = File('${dir.path}/$clean.mp3');
    if (target.path != oldFile.path && await target.exists()) throw StateError('A file with this name already exists.');
    final renamed = target.path == oldFile.path ? oldFile : await oldFile.rename(target.path);
    final updated = item.copyWith();
    final all = await _storage.loadHistory();
    final index = all.indexWhere((x) => x.id == item.id);
    if (index >= 0) {
      all[index] = HistoryItem(id: updated.id, title: clean, artist: updated.artist, thumbnail: updated.thumbnail,
        filename: renamed.uri.pathSegments.last, localPath: renamed.path, downloadedAt: updated.downloadedAt,
        fileSize: updated.fileSize, favorite: updated.favorite);
