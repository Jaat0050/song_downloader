...
class DownloadStorageService {
  ...
  Future<List<HistoryItem>> loadHistory() async {
    final prefs=await SharedPreferences.getInstance(); final raw=prefs.getStringList(_historyKey) ?? []; final items=<HistoryItem>[];
    for(final value in raw){try{items.add(HistoryItem.decode(value));}catch(_){}}
    items.removeWhere((item)=>item.localPath.isEmpty); items.sort((a,b)=>b.downloadedAt.compareTo(a.downloadedAt)); return items;
  }
  ...
  Future<void> updateHistoryItem(HistoryItem item) async { final current=await loadHistory(); final i=current.indexWhere((x)=>x.id==item.id); if(i<0)current.insert(0,item);else current[i]=item; await saveHistory(current); }
  ...
  Future<void> savePlaylists(Map<String,List<String>> playlists) async { final prefs=await SharedPreferences.getInstance(); await prefs.setString(_playlistsKey,jsonEncode(playlists)); }
}
